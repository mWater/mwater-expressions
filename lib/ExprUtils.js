var ExprUtils, _, addOpItem, i, k, l, len, len1, len2, len3, len4, len5, len6, len7, m, moment, n, o, op, opItems, p, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, relativeDateOp, relativeDateOps, type,
  indexOf = [].indexOf;
...
module.exports = ExprUtils = class ExprUtils {
  constructor(schema) {
    this.schema = schema;
  }
  ...
  findMatchingOpItems(search) {
    return _.filter(opItems, (opItem) => {
      ...
      if (search.lhsExpr) {
        lhsType = this.getExprType(search.lhsExpr);
...
// Get highest type
      for (i = 0, len = exprs.length; i < len; i++) {
        subExpr = exprs[i];
        if (this.getExprAggrStatus(subExpr) === "aggregate") {
          return "aggregate";
        }
      }
      for (k = 0, len1 = exprs.length; k < len1; k++) {
        subExpr = exprs[k];
        if (this.getExprAggrStatus(subExpr) === "individual") {
          return "individual";
        }
      }
      for (l = 0, len2 = exprs.length; l < len2; l++) {
        subExpr = exprs[l];
        if (this.getExprAggrStatus(subExpr) === "literal") {
          return "literal";
        }
      }
      return null;
  ...
  summarizeExpr(expr, locale) {
    ...
    switch (expr.type) {
      ...
      case "op":
        ...
        if (opItem) {
          if (opItem.prefix) {
            return opItem.name + " " + _.map(expr.exprs, (e) => {
