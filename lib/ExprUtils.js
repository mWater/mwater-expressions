...
var exprAggrStatusWeakCache = new WeakCache_1.WeakCache();
var ExprUtils = /** @class */ (function () {
    function ExprUtils(schema, variables) {
        this.schema = schema;
    ...
    ExprUtils.prototype.findMatchingOpItems = function (search) {
        ...
        return lodash_1.default.filter(items, function (opItem) {
            ...
            if (search.lhsExpr) {
                var lhsType = _this.getExprType(search.lhsExpr);
                if (lhsType) {
    ...
    ExprUtils.isOpPrefix = function (op) {
        return lodash_1.default.findWhere(opItems, { op: op, prefix: true }) != null;
    };
    ...
    ExprUtils.prototype.followJoins = function (startTable, joins) {
        ...
        for (var _i = 0, joins_1 = joins; _i < joins_1.length; _i++) {
            var j = joins_1[_i];
            var joinCol = this.schema.getColumn(t, j);
    ...
    ExprUtils.prototype.isMultipleJoins = function (table, joins) {
        var t = table;
        for (var _i = 0, joins_2 = joins; _i < joins_2.length; _i++) {
            var j = joins_2[_i];
            var joinCol = this.schema.getColumn(t, j);
            if (joinCol.type === "join") {
    ...
    ExprUtils.prototype.getExprEnumValues = function (expr) {
        ...
        if ((expr.type === "op") && ["last", "last where", "previous", "first", "first where"].includes(expr.op) && expr.exprs[0]) {
        ...
        if (expr.type === "case") {
            for (var _i = 0, _b = expr.cases; _i < _b.length; _i++) {
                var cse = _b[_i];
    ...
    ExprUtils.prototype.getExprAggrStatus = function (expr, _depth) {
        var _this = this;
        if ((expr == null) || !expr.type) {
            return null;
        }
        ...
        var getListAggrStatus = function (exprs) {
            // If has no expressions, is literal
            if (exprs.length === 0) {
                return "literal";
            }
            // Get highest type
            var aggrStatuses = lodash_1.default.map(exprs, function (subExpr) { return _this.getExprAggrStatus(subExpr, depth + 1); });
        ...
        switch (expr.type) {
            ...
            case "field":
                ...
                if (column && column.expr) {
                    ...
                    return exprAggrStatusWeakCache.cacheFunction([this.schema, column.expr], [this.variables], function () {
                        return _this.getExprAggrStatus(column.expr, depth + 1);
                    });
                }
                return "individual";
            case "op":
            ...
            case "case":
                // Gather all exprs
                var exprs = [expr.else];
                exprs = exprs.concat(lodash_1.default.map(expr.cases, function (cs) { return cs.when; }));
                exprs = exprs.concat(lodash_1.default.map(expr.cases, function (cs) { return cs.then; }));
    ...
    ExprUtils.prototype.areJoinsValid = function (table, joins) {
        var t = table;
        for (var _i = 0, joins_3 = joins; _i < joins_3.length; _i++) {
            var j = joins_3[_i];
            var joinCol = this.schema.getColumn(t, j);
            if (!joinCol) {
                return false;
    ...
    ExprUtils.prototype.getAggrTypes = function (expr) {
        var _a;
        var exprTable = this.getExprTable(expr);
        var aggrOpItems = this.findMatchingOpItems({ lhsExpr: expr, aggr: true, ordered: (exprTable ? ((_a = this.schema.getTable(exprTable)) === null || _a === void 0 ? void 0 : _a.ordering) != null : false) });
    ...
    ExprUtils.andExprs = function (table) {
        var exprs = [];
        for (var _i = 1; _i < arguments.length; _i++) {
            exprs[_i - 1] = arguments[_i];
        }
        var exprsMapped = lodash_1.default.map(exprs, function (expr) { if (((expr === null || expr === void 0 ? void 0 : expr.type) === "op") && (expr.op === "and")) {
            return expr.exprs;
        }
        else {
            return expr;
        ...
        return { type: "op", op: "and", table: table, exprs: exprsMapped };
    };
    ...
    ExprUtils.prototype.summarizeExpr = function (expr, locale) {
        ...
        switch (expr.type) {
            ...
            case "field":
                return this.localizeString((_a = this.schema.getColumn(expr.table, expr.column)) === null || _a === void 0 ? void 0 : _a.name, locale) || null;
            case "id":
                return this.localizeString((_b = this.schema.getTable(expr.table)) === null || _b === void 0 ? void 0 : _b.name, locale) || null;
            case "op":
                ...
                if (opItem) {
                    if (opItem.prefix) {
                        return (opItem.prefixLabel || opItem.name) + " " +
                            lodash_1.default.map(expr.exprs, function (e, index) {
                    ...
                    return lodash_1.default.map(expr.exprs, function (e) { return _this.summarizeExpr(e, locale); }).join(" " + opItem.name + " ");
                }
                else {
                    return "";
                }
    ...
    ExprUtils.prototype.summarizeScalarExpr = function (expr, locale) {
        var _a, _b;
        var exprType = this.getExprType(expr.expr);
        var str = "";
        // Add joins
        var t = expr.table;
        for (var _i = 0, _c = expr.joins; _i < _c.length; _i++) {
            var join = _c[_i];
            var joinCol = this.schema.getColumn(t, join);
            if (joinCol) {
    ...
    ExprUtils.prototype.stringifyLiteralValue = function (type, value, locale, enumValues, preferEnumCodes) {
        ...
        switch (type) {
            ...
            case "enum":
                ...
                return "???";
            case "enumset":
                return lodash_1.default.map(value, function (val) {
    ...
    ExprUtils.prototype.getReferencedFields = function (expr) {
        ...
        switch (expr.type) {
            ...
            case "case":
                ...
                break;
            case "scalar":
                for (var _d = 0, _e = expr.joins; _d < _e.length; _d++) {
                    var join = _e[_d];
            ...
            case "score":
                ...
                for (var _f = 0, _g = lodash_1.default.values(expr.scores); _f < _g.length; _f++) {
                    var value_1 = _g[_f];
                    cols = cols.concat(this.getReferencedFields(value_1));
                }
                break;
            case "build enumset":
                for (var _h = 0, _j = lodash_1.default.values(expr.values); _h < _j.length; _h++) {
                    var value_2 = _j[_h];
                    cols = cols.concat(this.getReferencedFields(value_2));
                }
                break;
            case "spatial join":
        ...
        return lodash_1.default.uniq(cols, function (col) { return col.table + "/" + col.column; });
    };
    /** Replace variables with literal values */
    ExprUtils.prototype.inlineVariableValues = function (expr, variableValues) {
        var _this = this;
        ...
        var replacer = function (part) {
            ...
            if (part && (part.type === "variable")) {
                // Find variable
                var variable = lodash_1.default.findWhere(_this.variables, { id: part.variableId });
                if (!variable) {
...
var addOpItem = function (item) {
    ...
    var list = groupedOpItems[item.op] || [];
    list.push(item);
...
for (var _i = 0, relativeDateOps_1 = relativeDateOps; _i < relativeDateOps_1.length; _i++) {
    var relativeDateOp = relativeDateOps_1[_i];
...
for (var _a = 0, _b = ['date', 'datetime']; _a < _b.length; _a++) {
    var type1 = _b[_a];
    for (var _c = 0, _d = ['date', 'datetime']; _c < _d.length; _c++) {
        var type2 = _d[_c];
...
addOpItem({ op: "yearquarter", name: "Year/Quarter", desc: "Year and quarter of a date", resultType: "enum", exprTypes: ["date"], prefix: true, rhsLiteral: false });
addOpItem({ op: "yearquarter", name: "Year/Quarter", desc: "Year and quarter of a date", resultType: "enum", exprTypes: ["datetime"], prefix: true, rhsLiteral: false });
addOpItem({ op: "yearweek", name: "Year/Week", desc: "Year and week of a date", resultType: "enum", exprTypes: ["date"], prefix: true, rhsLiteral: false });
addOpItem({ op: "yearweek", name: "Year/Week", desc: "Year and week of a date", resultType: "enum", exprTypes: ["datetime"], prefix: true, rhsLiteral: false });
addOpItem({ op: "weekofyear", name: "Week", desc: "Week of a date", resultType: "enum", exprTypes: ["date"], prefix: true, rhsLiteral: false });
addOpItem({ op: "weekofyear", name: "Week", desc: "Week of a date", resultType: "enum", exprTypes: ["datetime"], prefix: true, rhsLiteral: false });
addOpItem({ op: "within", name: "is within", resultType: "boolean", exprTypes: ["id", "id"], lhsCond: function (lhsExpr, exprUtils) {
        var lhsIdTable = exprUtils.getExprIdTable(lhsExpr);
        if (lhsIdTable) {
...
for (var _g = 0, _h = ['text', 'number', 'enum', 'enumset', 'boolean', 'date', 'datetime', 'geometry', 'id']; _g < _h.length; _g++) {
    var type = _h[_g];
...
for (var _j = 0, _k = ['number', 'date', 'datetime']; _j < _k.length; _j++) {
    var type = _k[_j];
...
addOpItem({ op: "within any", name: "is within any of", resultType: "boolean", exprTypes: ["id", "id[]"], lhsCond: function (lhsExpr, exprUtils) {
        var lhsIdTable = exprUtils.getExprIdTable(lhsExpr);
        if (lhsIdTable) {
            return (exprUtils.schema.getTable(lhsIdTable).ancestry != null) || (exprUtils.schema.getTable(lhsIdTable).ancestryTable != null);
        }
...
for (var _l = 0, _m = ['text', 'number', 'enum', 'enumset', 'boolean', 'date', 'datetime', 'geometry', 'image', 'imagelist', 'id', 'json', 'dataurl']; _l < _m.length; _l++) {
    var type = _m[_l];
...
for (var _o = 0, _p = ['id', 'text', 'date']; _o < _p.length; _o++) {
    var type = _p[_o];
...
addOpItem({ op: "is latest", name: "Is latest for each", desc: "Only include latest item for each of something", resultType: "boolean", exprTypes: ["id", "boolean"], prefix: true, ordered: true, aggr: false, rhsLiteral: false, joiner: "where", rhsPlaceholder: "All" });
