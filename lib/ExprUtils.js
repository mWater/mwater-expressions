var ExprUtils, _, addOpItem, aggrOpItems, groupedOpItems, i, k, l, len, len1, len2, len3, len4, len5, len6, len7, m, moment, n, o, op, opItems, p, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, relativeDateOp, relativeDateOps, type,
...
module.exports = ExprUtils = (function() {
  ...
  ExprUtils.prototype.findMatchingOpItems = function(search) {
    ...
    return _.filter(items, (function(_this) {
    ...
    getListAggrStatus = (function(_this) {
      return function(exprs) {
        ...
        aggrStatuses = _.map(exprs, function(subExpr) {
          return _this.getExprAggrStatus(subExpr, depth + 1);
        });
  ...
  ExprUtils.andExprs = function() {
    var exprs, table;
    table = arguments[0], exprs = 2 <= arguments.length ? slice.call(arguments, 1) : [];
  ...
  ExprUtils.prototype.summarizeExpr = function(expr, locale) {
    ...
    switch (expr.type) {
      ...
      case "op":
        ...
        if (opItem) {
          if (opItem.prefix) {
            return (opItem.prefixLabel || opItem.name) + " " + _.map(expr.exprs, (function(_this) {
            ...
            })(this)).join(opItem.joiner ? " " + opItem.joiner + " " : ", ");
          }
  ...
  ExprUtils.prototype.getReferencedFields = function(expr) {
    var cols, column, depth, i, join, k, l, len, len1, len2, len3, len4, m, n, ref, ref1, ref2, ref3, ref4, subcase, subexpr, table, value;
    cols = [];
    depth = arguments[1] || 0;
    if (depth > 100) {
    ...
    switch (expr.type) {
      case "field":
        ...
        column = this.schema.getColumn(expr.table, expr.column);
        if (column != null ? column.expr : void 0) {
          cols = cols.concat(this.getReferencedFields(column.expr, depth + 1));
        }
        break;
      case "op":
        ref = expr.exprs;
        for (i = 0, len = ref.length; i < len; i++) {
          subexpr = ref[i];
          cols = cols.concat(this.getReferencedFields(subexpr, depth + 1));
        }
        break;
      case "case":
        ref1 = expr.cases;
        for (k = 0, len1 = ref1.length; k < len1; k++) {
          subcase = ref1[k];
          cols = cols.concat(this.getReferencedFields(subcase.when, depth + 1));
          cols = cols.concat(this.getReferencedFields(subcase.then, depth + 1));
        }
        cols = cols.concat(this.getReferencedFields(expr["else"], depth + 1));
        break;
      case "scalar":
        table = expr.table;
        ref2 = expr.joins;
        ...
        cols = cols.concat(this.getReferencedFields(expr.expr, depth + 1));
        break;
      case "score":
        cols = cols.concat(this.getReferencedFields(expr.input, depth + 1));
        ref3 = _.values(expr.scores);
        for (m = 0, len3 = ref3.length; m < len3; m++) {
          value = ref3[m];
          cols = cols.concat(this.getReferencedFields(value, depth + 1));
        }
        break;
      case "build enumset":
