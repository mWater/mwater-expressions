...
module.exports = ExprEvaluator = class ExprEvaluator {
  ...
  evaluate(expr, context, callback) {
    ...
    switch (expr.type) {
      case "field":
        ...
        return context.row.getField(expr.column, (error, value) => {
          if (error) {
  ...
  evaluateOp(op, exprs, context, callback) {
    ...
    if (ExprUtils.isOpAggr(op)) {
      this.evaluteAggrOp(op, exprs, context, callback);
      return;
    }
    // is latest is special case for window-like function
    if (op === "is latest") {
      this.evaluateIsLatest(exprs, context, callback);
      return;
    }
    
    // Evaluate exprs
    return async.map(exprs, ((expr, cb) => {
      return this.evaluate(expr, context, cb);
    }), (error, values) => {
      var result;
      if (error) {
        return callback(error);
      ...
      try {
        result = this.evaluateOpValues(op, exprs, values);
  ...
  evaluateOpValues(op, exprs, values) {
    var date, exprUtils, hasNull, point, point1, point2;
  ...
  evaluteAggrOp(op, exprs, context, callback) {
    switch (op) {
      ...
      case "sum":
        // Evaluate all rows
        return async.map(context.rows, ((row, cb) => {
          return this.evaluate(exprs[0], {
            row: row
          }, cb);
        }), (error, values) => {
          if (error) {
...
// Take first non-null
  ...
  evaluateCase(expr, context, callback) {
    ...
    return async.map(expr.cases, ((acase, cb) => {
      return this.evaluate(acase.when, context, cb);
    }), (error, whens) => {
      if (error) {
        return callback(error);
      }
      return async.map(expr.cases, ((acase, cb) => {
        return this.evaluate(acase.then, context, cb);
      }), (error, thens) => {
        if (error) {
          return callback(error);
        }
        return this.evaluate(expr.else, context, (error, aelse) => {
  ...
  evaluateScalar(expr, context, callback) {
    ...
    return async.reduce(expr.joins, context, (memo, join, cb) => {
      ...
      if (memo.rows) {
        ...
        // Single row
        return memo.row.getField(join, (error, result) => {
          if (error) {
            return cb(error);
          }
          if (_.isArray(result)) {
      ...
      return this.evaluate(expr.expr, exprContext, callback);
    });
  }
  ...
  evaluateScore(expr, context, callback) {
    ...
    sum = 0;
    return this.evaluate(expr.input, context, (error, input) => {
      var scorePairs;
      if (error) {
        return callback(error);
      }
      scorePairs = _.pairs(expr.scores);
      return async.map(scorePairs, ((scorePair, cb) => {
        return this.evaluate(scorePair[1], context, cb);
      }), (error, values) => {
  ...
  evaluateBuildEnumset(expr, context, callback) {
    ...
    return async.map(valuePairs, ((valuePair, cb) => {
      return this.evaluate(valuePair[1], context, cb);
    }), (error, values) => {
      ...
      return async.map(context.rows, ((row, cb) => {
        ...
        return async.map(context.rows, ((row, cb) => {
          ...
          return async.map(context.rows, ((row, cb) => {
            ...
            // Get pk of row
            return context.row.getPrimaryKey((error, pk) => {
              if (error) {
                return callback(error);
              }
              // See if match
