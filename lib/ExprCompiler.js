...
var ColumnNotFoundException_1 = __importDefault(require("./ColumnNotFoundException"));
var ExprUtils_1 = __importDefault(require("./ExprUtils"));
var extensions_1 = require("./extensions");
var injectTableAliases_1 = require("./injectTableAliases");
...
var ExprCompiler = /** @class */ (function () {
    // Variable values are lookup of id to variable value, which is always an expression
    function ExprCompiler(schema, variables, variableValues) {
        this.schema = schema;
    ...
    ExprCompiler.prototype.compileExpr = function (options) {
        var expr = options.expr, tableAlias = options.tableAlias;
        // Handle null
        if (!expr) {
            return null;
        }
        switch (expr.type) {
            ...
            case "field":
                return this.compileFieldExpr({ expr: expr, tableAlias: tableAlias });
            case "scalar":
                return this.compileScalarExpr({ expr: expr, tableAlias: tableAlias });
            case "literal":
                if (expr.value != null) {
            ...
            case "op":
                return this.compileOpExpr({ expr: expr, tableAlias: tableAlias });
            case "case":
                return this.compileCaseExpr({ expr: expr, tableAlias: tableAlias });
            case "score":
                return this.compileScoreExpr({ expr: expr, tableAlias: tableAlias });
            case "build enumset":
                return this.compileBuildEnumsetExpr({ expr: expr, tableAlias: tableAlias });
            case "variable":
                return this.compileVariableExpr({ expr: expr, tableAlias: tableAlias });
            case "extension":
                return extensions_1.getExprExtension(expr.extension).compileExpr(expr, tableAlias, this.schema, this.variables, this.variableValues);
            case "count": // DEPRECATED
                return null;
            case "comparison": // DEPRECATED
                return this.compileComparisonExpr({ expr: expr, tableAlias: tableAlias });
            case "logical": // DEPRECATED
                return this.compileLogicalExpr({ expr: expr, tableAlias: tableAlias });
            default:
    ...
    ExprCompiler.prototype.compileScalarExpr = function (options) {
        ...
        if (!expr.aggr && !expr.where && (expr.joins.length === 1) && (expr.expr.type === "id")) {
            var fromColumn = this.schema.getColumn(expr.table, expr.joins[0]);
        ...
        var generateAlias = function (expr, joinIndex) {
        ...
        if (expr.joins && (expr.joins.length > 0)) {
            ...
            var alias = generateAlias(expr, 0);
        ...
        if (expr.joins.length > 1) {
            for (var i = 1, end = expr.joins.length, asc = 1 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
                ...
                toTable = joinColumn.type === "join" ? joinColumn.join.toTable : joinColumn.idTable;
                // Generate a consistent, semi-unique alias
                var nextAlias = generateAlias(expr, i);
                var onClause = this.compileJoin(table, joinColumn, tableAlias, nextAlias);
                from = {
                    type: "join",
                    left: from,
        ...
        if (expr.where) {
            var extraWhere = this.compileExpr({ expr: expr.where, tableAlias: tableAlias });
            // Add to existing 
            if (where) {
        ...
        var scalarExpr = this.compileExpr({ expr: expr.expr, tableAlias: tableAlias });
        // Aggregate DEPRECATED
        if (expr.aggr) {
            switch (expr.aggr) {
                case "last":
                    // Get ordering
                    var ordering = this.schema.getTable(table).ordering;
                    if (!ordering) {
                        throw new Error("No ordering defined");
                    }
                    // order descending
                    orderBy = [{ expr: this.compileFieldExpr({ expr: { type: "field", table: table, column: ordering }, tableAlias: tableAlias }), direction: "desc" }];
                    break;
                case "sum":
                case "count":
                case "avg":
     ...
     */
    ExprCompiler.prototype.compileJoin = function (fromTableId, joinColumn, fromAlias, toAlias) {
        // For join columns
        var toTable;
        if (joinColumn.type === "join") {
            if (joinColumn.join.jsonql) {
                return injectTableAliases_1.injectTableAliases(joinColumn.join.jsonql, { "{from}": fromAlias, "{to}": toAlias });
            }
            else {
                // Use manual columns
                return {
            ...
            // Get to table
            toTable = this.schema.getTable(joinColumn.idTable);
            var compiledFrom = this.compileFieldExpr({ expr: { type: "field", table: fromTableId, column: joinColumn.id }, tableAlias: fromAlias });
            var compiledTo = { type: "field", tableAlias: toAlias, column: toTable.primaryKey };
    ...
    ExprCompiler.prototype.compileOpExpr = function (options) {
        ...
        var expr = options.expr;
        var compiledExprs = lodash_1.default.map(expr.exprs, function (e) { return _this.compileExpr({ expr: e, tableAlias: options.tableAlias }); });
        // Get type of expr 0
        var expr0Type = exprUtils.getExprType(expr.exprs[0]);
        // Handle multi
        switch (expr.op) {
            case "and":
            case "or":
            ...
            case "*":
                ...
                return {
                    type: "op",
                    op: expr.op,
                    exprs: lodash_1.default.map(compiledExprs, function (e) { return ({
                        type: "op",
                        op: "::decimal",
                        exprs: [e]
                    }); })
            ...
            case "-":
                ...
                if (lodash_1.default.any(compiledExprs, function (ce) { return ce == null; })) {
                    return null;
                }
            ...
            case '= any':
                ...
                if (expr.exprs[1].type === "literal") {
                    var rhsLiteral = expr.exprs[1];
                    if (rhsLiteral.value == null || (lodash_1.default.isArray(rhsLiteral.value) && (rhsLiteral.value.length === 0))) {
                        return false;
                    }
                }
            ...
            case "to text":
                ...
                if (exprUtils.getExprType(expr.exprs[0]) === "enum") {
                    ...
                    var enumValues = exprUtils.getExprEnumValues(expr.exprs[0]);
                    if (!enumValues) {
                        return null;
                    }
                    return {
                        ...
                        cases: lodash_1.default.map(enumValues, function (ev) {
                            return {
                        ...
                        compiledExprs[0],
                        {
                            ...
                            where: {
                                ...
                                exprs: [
                                    ...
                                    { type: "literal", value: lodash_1.default.map(compiledExprs[1].value, function (value) {
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "-",
                            exprs: [
                                { type: "op", op: "::date", exprs: [moment_1.default().format("YYYY-MM-DD")] },
                                { type: "op", op: "::date", exprs: [compiledExprs[0]] }
                            ]
                        };
                    case "datetime":
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("year").format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("year").add(1, 'years').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("year").toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("year").add(1, 'years').toISOString()] }
                            ]
                        };
                    default:
                        return null;
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("year").subtract(1, 'years').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("year").format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("year").subtract(1, 'years').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("year").toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("month").format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("month").add(1, 'months').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("month").toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("month").add(1, 'months').toISOString()] }
                            ]
                        };
                    default:
                        return null;
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("month").subtract(1, 'months').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("month").format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("month").subtract(1, 'months').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("month").toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("day").toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                        return null;
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(1, 'days').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("day").subtract(1, 'days').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(7, 'days').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("day").subtract(7, 'days').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(30, 'days').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("day").subtract(30, 'days').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(365, 'days').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().startOf("day").subtract(365, 'days').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(11, "months").startOf('month').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(11, "months").startOf('month').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(5, "months").startOf('month').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(5, "months").startOf('month').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(2, "months").startOf('month').format("YYYY-MM-DD")] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().add(1, 'days').format("YYYY-MM-DD")] }
                            ]
                        };
                    case "datetime":
                        return {
                            ...
                            op: "and",
                            exprs: [
                                { type: "op", op: ">=", exprs: [compiledExprs[0], moment_1.default().subtract(2, "months").startOf('month').toISOString()] },
                                { type: "op", op: "<", exprs: [compiledExprs[0], moment_1.default().startOf("day").add(1, 'days').toISOString()] }
                            ]
                        };
                    default:
                ...
                switch (expr0Type) {
                    case "date":
                        return {
                            type: "op",
                            op: ">",
                            exprs: [compiledExprs[0], moment_1.default().format("YYYY-MM-DD")]
                        };
                    case "datetime":
                        return {
                            type: "op",
            ...
            case 'current date':
                return { type: "literal", value: moment_1.default().format("YYYY-MM-DD") };
            case 'current datetime':
                return { type: "literal", value: moment_1.default().toISOString() };
            case 'distance':
                if (compiledExprs[0] == null || compiledExprs[1] == null) {
                    return null;
                }
            ...
            case 'is latest':
                ...
                // order descending
                var orderBy = [{ expr: this.compileFieldExpr({ expr: { type: "field", table: expr.table, column: ordering }, tableAlias: "innerrn" }), direction: "desc" }];
    ...
    ExprCompiler.prototype.compileCaseExpr = function (options) {
        ...
        var compiled = {
            type: "case",
            cases: lodash_1.default.map(expr.cases, function (c) {
                return {
                    when: _this.compileExpr({ expr: c.when, tableAlias: options.tableAlias }),
                    then: _this.compileExpr({ expr: c.then, tableAlias: options.tableAlias })
                };
            }),
            else: this.compileExpr({ expr: expr.else, tableAlias: options.tableAlias })
        };
        // Remove null cases
        compiled.cases = lodash_1.default.filter(compiled.cases, function (c) { return c.when != null; });
    ...
    ExprCompiler.prototype.compileScoreExpr = function (options) {
        ...
        // Get type of input
        var inputType = exprUtils.getExprType(expr.input);
        switch (inputType) {
            case "enum":
                return {
                    type: "case",
                    input: this.compileExpr({ expr: expr.input, tableAlias: options.tableAlias }),
                    cases: lodash_1.default.map(lodash_1.default.pairs(expr.scores), function (pair) {
                        return {
                            when: { type: "literal", value: pair[0] },
                            then: _this.compileExpr({ expr: pair[1], tableAlias: options.tableAlias })
                        };
                    }),
            ...
            case "enumset":
                return {
                    type: "op",
                    op: "+",
                    exprs: lodash_1.default.map(lodash_1.default.pairs(expr.scores), function (pair) {
                        return {
                            type: "case",
                            cases: [
                                {
                                    when: {
                                        ...
                                        op: "@>",
                                        exprs: [
                                            convertToJsonB(_this.compileExpr({ expr: expr.input, tableAlias: options.tableAlias })),
    ...
    ExprCompiler.prototype.compileBuildEnumsetExpr = function (options) {
        ...
        return {
            ...
            from: {
                ...
                query: {
                    type: "union all",
                    queries: lodash_1.default.map(lodash_1.default.pairs(expr.values), function (pair) {
                        return {
                            type: "query",
                            selects: [
                                {
                                    ...
                                    expr: {
                                        type: "case",
                                        cases: [{ when: _this.compileExpr({ expr: pair[1], tableAlias: options.tableAlias }), then: pair[0] }]
                                    },
                                    alias: "v"
                                }
                            ]
    ...
    ExprCompiler.prototype.compileComparisonExpr = function (options) {
        ...
        var exprLhsType = exprUtils.getExprType(expr.lhs);
        if (!exprLhsType) {
            return null;
        }
        ...
        var lhsExpr = this.compileExpr({ expr: expr.lhs, tableAlias: options.tableAlias });
        if (expr.rhs) {
            var rhsExpr = this.compileExpr({ expr: expr.rhs, tableAlias: options.tableAlias });
    ...
    ExprCompiler.prototype.compileVariableExpr = function (options) {
        // Get variable
        var variable = lodash_1.default.findWhere(this.variables, { id: options.expr.variableId });
        if (!variable) {
