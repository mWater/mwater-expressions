...
var ColumnNotFoundException, ExprCompiler, ExprUtils, _, injectTableAlias, injectTableAliases, moment, nowExpr, nowMinus24HoursExpr;
  ...
  (0, _createClass2["default"])(ExprCompiler, [{
    ...
    value: function compileScalarExpr(options) {
      ...
      if (expr.joins && expr.joins.length > 0) {
        ...
        join = joinColumn.join; // Generate a consistent, semi-unique alias

        alias = generateAlias(expr, 0);
        where = this.compileJoin(join, tableAlias, alias);
        from = this.compileTable(join.toTable, alias); // We are now at j1, which is the to of the first join
    ...
    key: "compileJoin",
    value: function compileJoin(join, fromAlias, toAlias) {
      if (join.jsonql) {
