...
var ColumnNotFoundException, ExprCompiler, ExprUtils, _, convertToJsonB, getExprExtension, injectTableAlias, injectTableAliases, moment, nowExpr, nowMinus24HoursExpr;
...
getExprExtension = require('./extensions').getExprExtension; // now expression: (to_json(now() at time zone 'UTC')#>>'{}')
  ...
  (0, _createClass2["default"])(ExprCompiler, [{
    ...
    value: function compileExpr(options) {
      ...
      switch (expr.type) {
        ...
        case "extension":
          compiledExpr = getExprExtension(expr.extension).compileExpr(expr, options.tableAlias, this.schema, this.variables, this.variableValues);
          break;
    ...
    value: function compileScalarExpr(options) {
      var alias, expr, extraWhere, from, fromColumn, generateAlias, i, j, joinColumn, nextAlias, onClause, orderBy, ordering, ref, scalar, scalarExpr, table, tableAlias, toTable, where;
      expr = options.expr;
      where = null;
      from = null;
      ...
      if (!expr.aggr && !expr.where && expr.joins.length === 1 && expr.expr.type === "id") {
        fromColumn = this.schema.getColumn(expr.table, expr.joins[0]);

        if (fromColumn.type === "id" || fromColumn.join && fromColumn.join.toColumn === this.schema.getTable(expr.expr.table).primaryKey) {
        ...
        case "line length":
          ...
          return {
            ...
            exprs: [{
              ...
              type: "op",
              op: "::spheroid",
              exprs: ['SPHEROID["GRS_1980",6378137,298.257222101]']
            }]
          };
