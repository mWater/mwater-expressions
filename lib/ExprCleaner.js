...
var ExprCleaner = /** @class */ (function () {
    function ExprCleaner(schema, variables) {
    ...
    ExprCleaner.prototype.cleanVariableExpr = function (expr, options) {
        // Get variable
        var variable = this.variables.find(function (v) { return v.id == expr.variableId; });
        if (!variable) {
            return null;
        }
        // Check id table
    ...
    ExprCleaner.prototype.cleanSpatialJoinExpr = function (expr, options) {
        var _this = this;
        return immer_1.default(expr, function (draft) {
            // Clean geometry from
            draft.fromGeometryExpr = _this.cleanExpr(expr.fromGeometryExpr, { types: ['geometry'], table: options.table });
            // If toTable, clean rest
            if (expr.toTable) {
                if (!_this.schema.getTable(expr.toTable)) {
                    draft.toTable = null;
                }
                else {
                    draft.toGeometryExpr = _this.cleanExpr(expr.toGeometryExpr, { types: ['geometry'], table: expr.toTable });
                    draft.valueExpr = _this.cleanExpr(expr.valueExpr, { aggrStatuses: ["aggregate"], types: options.types, table: expr.toTable });
                    draft.filterExpr = _this.cleanExpr(expr.filterExpr, { types: ["boolean"], table: expr.toTable });
                }
            }
        });
    };
