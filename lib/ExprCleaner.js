        ...
        switch (expr.op) {
            ...
            default:
                ...
                return immer_1.default(expr, (draft) => {
                    ...
                    if (expr.exprs.length > opItem.exprTypes.length && !opItem.moreExprType) {
                    ...
                    for (let i = 0; i < draft.exprs.length; i++) {
                        draft.exprs[i] = this.cleanExpr(expr.exprs[i] || null, {
                            table: expr.table,
                            types: opItem.exprTypes[i] ? [opItem.exprTypes[i]] : (opItem.moreExprType ? [opItem.moreExprType] : undefined),
                            enumValueIds,
