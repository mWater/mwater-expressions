var ExprCleaner, ExprUtils, ExprValidator, _,
  indexOf = [].indexOf;
...
module.exports = ExprCleaner = class ExprCleaner {
  constructor(schema) {
  ...
  cleanExpr(expr, options = {}) {
    var aggrOpItems, aggrStatus, args, booleanOnly, ex, i, k, l, opItem, ref, ref1, ref2, ref3, type;
    _.defaults(options, {
...
// Add extra nulls for other arguments
  ...
  cleanOpExpr(expr, options) {
    ...
    switch (expr.op) {
      ...
      case "or":
        expr = _.extend({}, expr, {
          exprs: _.map(expr.exprs, (e) => {
            return this.cleanExpr(e, {
              types: ["boolean"],
              table: expr.table
            });
          })
  ...
  cleanCaseExpr(expr, options) {
    ...
    expr = _.extend({}, expr, {
      cases: _.map(expr.cases, (c) => {
        return _.extend({}, c, {
          when: this.cleanExpr(c.when, {
            types: ["boolean"],
            table: expr.table
          }),
          then: this.cleanExpr(c.then, options)
        });
      }),
  ...
  cleanScoreExpr(expr, options) {
    ...
    // Clean score values
    expr = _.extend({}, expr, {
      scores: _.mapValues(expr.scores, (scoreExpr) => {
        return this.cleanExpr(scoreExpr, {
          table: expr.table,
          types: ['number']
        });
      })
    ...
    if (expr.input) {
      ...
      expr = _.extend({}, expr, {
        scores: _.pick(expr.scores, (value, key) => {
  ...
  cleanBuildEnumsetExpr(expr, options) {
    // Clean values
    expr = _.extend({}, expr, {
      values: _.mapValues(expr.values, (valueExpr) => {
        return this.cleanExpr(valueExpr, {
          table: expr.table,
          types: ['boolean']
        });
      })
    ...
    if (options.enumValueIds) {
      expr = _.extend({}, expr, {
        values: _.pick(expr.values, (value, key) => {
  ...
  cleanComparisonExpr(expr, options) {
    ...
    newExpr.exprs = _.map(newExpr.exprs, (e) => {
      return this.cleanExpr(e);
    });
    // If = true
    if (expr.op === "= true") {
      ...
      if (this.exprUtils.getExprType(expr.lhs) === "date") {
        newExpr.exprs = [
          expr.lhs,
          {
            type: "literal",
            valueType: "date",
            value: expr.rhs.value[0].substr(0,
          10)
          },
          {
            type: "literal",
            valueType: "date",
            value: expr.rhs.value[1].substr(0,
          10)
          }
        ];
      } else {
