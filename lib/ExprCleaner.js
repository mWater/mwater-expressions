...
module.exports = ExprCleaner = (function() {
  ...
  ExprCleaner.prototype.cleanOpExpr = function(expr, options) {
    var enumValueIds, enumValues, exprs, lhsExpr, opItem, opItems, ref;
    switch (expr.op) {
      case "and":
      case "or":
        expr = _.extend({}, expr, {
      ...
      default:
        ...
        if (!lhsExpr && !((ref = opItems[0]) != null ? ref.prefix : void 0)) {
          return null;
        }
        if (opItems.length > 1) {
  ...
  ExprCleaner.prototype.cleanScalarExpr = function(expr, options) {
    var aggrs, innerTable, innerTypes, joins, ref;
    ...
    if (expr.aggr && (!this.exprUtils.isMultipleJoins(expr.table, expr.joins) || !expr.expr)) {
      expr = _.omit(expr, "aggr");
    }
    if (expr.expr && this.exprUtils.isMultipleJoins(expr.table, expr.joins) && (ref = expr.aggr, indexOf.call(_.pluck(this.exprUtils.getAggrs(expr.expr), "id"), ref) < 0)) {
