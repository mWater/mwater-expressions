...
describe("ExprUtils", function() {
  ...
  it("localizes strings", function() {
    ...
    assert.equal(this.exprUtils.localizeString({ _base: "fr", en: "apple", fr: "pomme" }, null), "pomme", "_base wins if no locale");
    return assert.equal(this.exprUtils.localizeString({ _base: "fr", en: "apple", fr: "pomme" }, "en"), "apple", "_base wins if no locale");
  });
  ...
  describe("getExprIdTable", function() {
    it("gets for literal", function() {
      return assert.equal(this.exprUtils.getExprIdTable({ type: "literal", valueType: "id", idTable: "xyz", value: "123" }), "xyz");
    });
    ...
    it("gets for scalar", function() {
      return assert.equal(this.exprUtils.getExprIdTable({ type: "scalar", table: "t2", joins: ["2-1"], expr: { type: "id", table: "t1" }}), "t1");
    });

    return it("gets for variable", function() {
      return assert.equal(this.exprUtils.getExprIdTable({ type: "variable", table: "t1", variableId: "varidexpr" }), "t2");
    });
  });

  describe("getExprAggrStatus", function() {
    it("gets for literal", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "literal", valueType: "id", idTable: "xyz", value: "123" }), "literal");
    });
    ...
    it("gets for field", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "field", table: "t1", column: "number" }), "individual");
    });

    it("gets for expr field", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "field", table: "t1", column: "expr_sum" }), "aggregate");
    });

    it("gets for aggregate", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "op", op: "sum", exprs: [{ type: "field", table: "xyz", column: "abc" }]}), "aggregate");
    });

    it("gets for aggregate + literal", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "op", op: "+", exprs: [
    ...
    it("gets for scalar", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "scalar", table: "t2", joins: ["2-1"], expr: { type: "id", table: "t1" }}), "individual");
    });

    it("gets for scalar aggregation", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ 
        type: "scalar", 
        table: "t1", 
        joins: ["1-2"], 
        expr: { 
    ...
    return it("gets for individual variable", function() {
      return assert.equal(this.exprUtils.getExprAggrStatus({ type: "variable", table: "t1", variableId: "varidexpr" }), "individual");
    });
  });

  describe("findMatchingOpItems", function() {
    it("finds = for number", function() {
      return assert.equal(this.exprUtils.findMatchingOpItems({lhsExpr: { type: "field", table: "t1", column: "number" }})[0].op, "=");
    });

    it("finds = for expr number", function() {
      return assert.equal(this.exprUtils.findMatchingOpItems({lhsExpr: { type: "field", table: "t1", column: "expr_number" }})[0].op, "=");
    });
    ...
    return it("first within for id type hierarchical", function() {
      return assert.equal(this.exprUtils.findMatchingOpItems({lhsExpr: { type: "id", table: "thier" }})[0].op, "within");
    });
  });
  ...
  describe("getExprType", function() {
    ...
    it("number type if number + number", function() {
      return assert.equal(this.exprUtils.getExprType({ type: "op", op: "+", exprs: [{ type: "field", table: "t1", column: "number" }, { type: "field", table: "t1", column: "number" }]}), "number");
    });
  ...
  describe("summarizeExpr", function() {
    ...
    it("simplifies when count id", function() {
      const scalarExpr = { type: "scalar", table: "t1", joins: ['1-2'], expr: { type: "id", table: "t2" }, aggr: "count" };
    ...
    it("shows join types", function() {
      const scalarExpr = { type: "scalar", table: "t3", joins: ['3-2'], expr: { type: "field", table: "t2", column: "2-1" } };
    ...
    it("summarizes max where expr", function() {
      const expr = { type: "op", op: "max where", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, { type: "field", table: "t1", column: "boolean" }] };
    ...
    it("summarizes max where expr without rhs", function() {
      const expr = { type: "op", op: "max where", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, null] };
  ...
  describe("findMatchingOpItems", () => it("finds = any for text lhs with boolean result", function() {
  ...
  describe("getExprEnumValues", function() {
    it("finds in field", function() {
      return assert.deepEqual(this.exprUtils.getExprEnumValues({ type: "field", table: "t1", column: "enum" }), [{ id: "a", name: { en: "A" }}, { id: "b", name: { en: "B" }}]);
  });
    ...
    it("finds in case statement thens", function() {
      ...
      return assert.deepEqual(this.exprUtils.getExprEnumValues(expr), [{ id: "a", name: { en: "A" }}, { id: "b", name: { en: "B" }}]);
  });
    ...
    return it("finds in field", function() {
      return assert.deepEqual(this.exprUtils.getExprEnumValues({ type: "variable", variableId: "varenum" }), [{ id: "a", name: { en: "A" }}, { id: "b", name: { en: "B" }}]);
  });
});
  ...
  describe("getReferencedFields", function() {
    ...
    it("gets expr field", function() {
      ...
      return compare(cols, [{ type: "field", table: "t1", column: "expr_number" }, { type: "field", table: "t1", column: "number" }]);
    });

    it("gets join", function() {
      const cols = this.exprUtils.getReferencedFields({ type: "scalar", table: "t1", joins: ["1-2"], expr: { type: "op", table: "t2", exprs: [{ type: "field", table: "t2", column: "number" }]}});
      return compare(cols, [{ type: "field", table: "t1", column: "1-2" }, { type: "field", table: "t2", column: "number" }]);
    });

    it("recurses into ops", function() {
      const cols = this.exprUtils.getReferencedFields({ type: "op", op: "+", exprs: [{ type: "field", table: "t1", column: "number" }]});
      return compare(cols, [{ type: "field", table: "t1", column: "number" }]);
    });
    ...
    it("includes cases", function() {
      ...
      return compare(cols, [{ type: "field", table: "t1", column: "boolean" }, { type: "field", table: "t1", column: "enum" }, { type: "field", table: "t1", column: "text" }]);
    });
    ...
    return it("de-duplicates", function() {
      const cols = this.exprUtils.getReferencedFields({ type: "op", op: "+", exprs: [{ type: "field", table: "t1", column: "number" }, { type: "field", table: "t1", column: "number" }]});
      return compare(cols, [{ type: "field", table: "t1", column: "number" }]);
    });
  });
  ...
  describe("inlineVariableValues", function() {
    it("inlines literals", function() {
      const expr = { type: "op", op: ">", exprs: [
        { type: "variable", variableId: "varnumber" },
        { type: "literal", valueType: "number", value: 3 }
      ]};
      const result = { type: "op", op: ">", exprs: [
        { type: "literal", valueType: "number", value: 4 },
        { type: "literal", valueType: "number", value: 3 }
      ]};
      return compare(this.exprUtils.inlineVariableValues(expr, { varnumber: { type: "literal", valueType: "number", value: 4 } }), result);
    });
    ...
    it("inlines literal ids", function() {
      ...
      return compare(this.exprUtils.inlineVariableValues(expr, { varid: { type: "literal", valueType: "id", idTable: "t2", value: "123" }}), result);
    });
  ...
  return describe("andExprs", function() {
    ...
    return it("denests", () => compare(ExprUtils.andExprs("t1", { type: "field", table: "t1", column: "b1" }, { type: "op", op: "and", exprs: [{ type: "field", table: "t1", column: "b2" }, { type: "field", table: "t1", column: "b3" }] }),
