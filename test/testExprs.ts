...
function literal(value: any, type: any) {
  return {
    type: "literal",
    valueType: type,
    value
...
function addOp(result: any, op: any, ...exprs: any[]) {
...
function makeRow(data: any) {
  return {
    ...
    getField(columnId: any) {
      return Promise.resolve(data[columnId])
    },
...
addOp(
  0,
  "distance",
  literal({ type: "Point", coordinates: [1, 2] }, "geometry"),
  literal({ type: "Point", coordinates: [1, 2] }, "geometry")
)
addOp(
  (v: any) => v > 310000 && v < 320000,
  "distance",
  literal({ type: "Point", coordinates: [1, 2] }, "geometry"),
  literal({ type: "Point", coordinates: [3, 4] }, "geometry")
)
...
addOp(
  1.5,
  "days difference",
  literal("2016-06-23T17:36:51.412Z", "datetime"),
  literal("2016-06-22T05:36:51.412Z", "datetime")
)
...
addOp(
  30 / 30.5,
  "months difference",
  literal("2016-07-22T05:36:51.412Z", "datetime"),
  literal("2016-06-22T05:36:51.412Z", "datetime")
)
...
addOp(
  1,
  "years difference",
  literal("2015-07-22T05:36:51.412Z", "datetime"),
  literal("2014-07-22T05:36:51.412Z", "datetime")
)
addOp(null, "years difference", literal("2016-06-23T17:36:51.412Z", "datetime"), literal(null, "datetime"))

addOp((v: any) => v > 0.9 && v < 1.1, "days since", literal(moment().subtract(1, "days").toISOString(), "datetime"))
...
add({ type: "op", table: "t1", op: "to text", exprs: [{ type: "field", table: "t1", column: "enum" }] }, "A", {
  row: sampleRow
})
...
addOp((d: any) => d.startsWith(moment().toISOString().substr(0, 10)), "current datetime")
...
add({ type: "op", table: "t1", op: "sum", exprs: [{ type: "field", table: "t1", column: "a" }] }, 10, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "avg", exprs: [{ type: "field", table: "t1", column: "a" }] }, 2.5, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "min", exprs: [{ type: "field", table: "t1", column: "a" }] }, 1, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "max", exprs: [{ type: "field", table: "t1", column: "a" }] }, 4, {
  rows: sampleRows
})
...
add({ type: "op", table: "t1", op: "last", exprs: [{ type: "field", table: "t1", column: "a" }] }, 2, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "last", exprs: [{ type: "field", table: "t1", column: "f" }] }, 1, {
  rows: sampleRows
})

add({ type: "op", table: "t1", op: "previous", exprs: [{ type: "field", table: "t1", column: "a" }] }, 1, {
  rows: sampleRows
})
...
add(
  {
    ...
    op: "last where",
    exprs: [
      { type: "field", table: "t1", column: "a" },
      { type: "field", table: "t1", column: "c" }
    ]
  },
  1,
  { rows: sampleRows }
)
add({ type: "op", table: "t1", op: "last where", exprs: [{ type: "field", table: "t1", column: "a" }, null] }, 2, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "last where", exprs: [{ type: "field", table: "t1", column: "f" }, null] }, 1, {
  rows: sampleRows
})

add({ type: "op", table: "t1", op: "first", exprs: [{ type: "field", table: "t1", column: "a" }] }, 3, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "first", exprs: [{ type: "field", table: "t1", column: "f" }] }, 3, {
  rows: sampleRows
})
...
add(
  {
    ...
    exprs: [
      { type: "field", table: "t1", column: "a" },
      { type: "op", table: "t1", op: "not", exprs: [{ type: "field", table: "t1", column: "c" }] }
    ]
  },
  4,
  { rows: sampleRows }
)
add({ type: "op", table: "t1", op: "first where", exprs: [{ type: "field", table: "t1", column: "a" }, null] }, 3, {
  rows: sampleRows
})
add({ type: "op", table: "t1", op: "first where", exprs: [{ type: "field", table: "t1", column: "f" }, null] }, 3, {
  rows: sampleRows
})

add({ type: "op", table: "t1", op: "count where", exprs: [{ type: "field", table: "t1", column: "c" }] }, 2, {
  rows: sampleRows
})
...
add({ type: "op", table: "t1", op: "percent where", exprs: [{ type: "field", table: "t1", column: "c" }] }, 50, {
  rows: sampleRows
})
add(
  {
    ...
    exprs: [
      { type: "field", table: "t1", column: "c" },
      { type: "field", table: "t1", column: "d" }
    ]
  },
  (v: any) => Math.abs(v - 200 / 3) < 0.1,
  { rows: sampleRows }
)

add({ type: "op", table: "t1", op: "count distinct", exprs: [{ type: "field", table: "t1", column: "e" }] }, 3, {
  rows: sampleRows
})

add(
  { type: "op", table: "t1", op: "array_agg", exprs: [{ type: "field", table: "t1", column: "e" }] },
  ["x", "y", "z", "z"],
  { rows: sampleRows }
)
...
add(
  {
    type: "scalar",
    joins: ["j"],
    expr: { type: "op", table: "t2", op: "sum", exprs: [{ type: "field", table: "t2", column: "a" }] }
  },
  10,
  { row: multipleJoinRow }
)
...
add(
  {
    type: "scalar",
    joins: ["j1", "j2"],
    expr: { type: "op", table: "t3", op: "sum", exprs: [{ type: "field", table: "t3", column: "a" }] }
  },
  10,
  { row: multipleJoinsRow }
)
...
// Scalar with no row
add({ type: "scalar", joins: ["j"], expr: { type: "field", table: "t2", column: "b" } }, null, {
  row: makeRow({ "join:j": null, j: null })
})
...
add({ type: "op", op: "is latest", table: "t1", exprs: [{ type: "field", table: "t1", column: "a" }] }, true, {
  row: sampleRows[0],
  rows: sampleRows
})
add({ type: "op", op: "is latest", table: "t1", exprs: [{ type: "field", table: "t1", column: "a" }] }, true, {
...
add({ type: "op", op: "is latest", table: "t1", exprs: [{ type: "field", table: "t1", column: "a" }] }, false, {
  row: sampleRows[2],
  rows: sampleRows
})
add({ type: "op", op: "is latest", table: "t1", exprs: [{ type: "field", table: "t1", column: "a" }] }, false, {
