...
describe("ExprCleaner", function() {
  beforeEach(function() {
    ...
    return this.clean = (expr, expected, options) => {
  ...
  describe("cleanExpr", function() {
    ...
    it("allows variable if right id table", function() {
      assert.isNotNull(this.exprCleaner.cleanExpr({ type: "variable", variableId: "varid" }, { table: "t2", idTable: "t1" }));
      return assert.isNull(this.exprCleaner.cleanExpr({ type: "variable", variableId: "varid" }, { table: "t2", idTable: "t2" }));
    });
    ...
    it("cleans aggregate op", function() {
      const expr = { type: "op", table: "t1", op: "and", exprs: [
        { type: "op", op: "=", table: "t1", exprs: [{ type: "op", table: "t1", op: "count", exprs: [] }, null] },
        null
      ]};
      return compare(this.exprCleaner.cleanExpr(expr, {aggrStatuses: ['aggregate', 'individual', 'literal'], types: ['boolean']}), expr);
    });
    ...
    describe("aggregation", function() {
      ...
      it("nulls inner expr if wrong aggregation status", function() {
        ...
        return compare(this.exprCleaner.cleanExpr(expr, {aggrStatuses: ["aggregate"]}), { type: "op", op: "sum", table: "t1", exprs: [null] });
      });
      ...
      return it("allows no args for count", function() {
        ...
        return compare(this.exprCleaner.cleanExpr(expr, {types: ["number"], aggrStatuses: ["aggregate"]}), { type: "op", table: "t1", op: "count", exprs: [] });
      });
    });
    ...
    describe("fixing expression types", function() {
      it("creates boolean from enum", function() {
        ...
        return compare(this.exprCleaner.cleanExpr(expr, {types: ["boolean"]}), { type: "op", table: "t1", op: "= any", exprs: [expr, null] });
      });
      ...
      return it("creates boolean inside percent where from enum", function() {
        const expr = { type: "op", table: "t1", op: "percent where", exprs: [{ type: "field", table: "t1", column: "enum" }] };
        return compare(
          this.exprCleaner.cleanExpr(expr, {types: ["number"], aggrStatuses: ["aggregate", "literal"]}), 
          {
            type: "op",
    ...
    describe("op", function() {
      it("preserves 'and' by cleaning child expressions with boolean type", function() {
        const expr = { type: "op", op: "and", table: "t1", exprs: [{ type: "field", table: "t1", column: "enum" }, { type: "field", table: "t1", column: "boolean" }]};
      ...
      it("does not allow enum = enumset", function() {
        const expr = { type: "op", op: "=", table: "t1", exprs: [{ type: "field", table: "t1", column: "enum" }, { type: "literal", valueType: "enumset", value: ["a"] }]};
        return compare(this.exprCleaner.cleanExpr(expr).exprs[1], null);
      });

      it("defaults op if lhs changes", function() {
        const expr = { type: "op", op: "= any", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, { type: "literal", valueType: "enumset", value: ["a"] }]};
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "=", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, null]});
      });

      it("removes extra exprs", function() {
        const expr = { type: "op", op: "=", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, null, null]};
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "=", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, null]});
      });
      ...
      it("allows null=wildcard unary expressions", function() {
        const expr = { type: "op", op: "is null", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }, null]};
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "is null", table: "t1", exprs: [{ type: "field", table: "t1", column: "number" }]});
      });

      it("removes invalid enums on rhs", function() {
        const expr = { type: "op", op: "= any", table: "t1", exprs: [{ type: "field", table: "t1", column: "enum" }, { type: "literal", valueType: "enumset", value: ["a", "x"] }]};
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "= any", table: "t1", exprs: [{ type: "field", table: "t1", column: "enum" }, { type: "literal", valueType: "enumset", value: ["a"] }]});
      }); // x is gone

      it("removes invalid id table on rhs", function() {
        let expr = { type: "op", op: "=", table: "t1", exprs: [{ type: "id", table: "t1" }, { type: "literal", valueType: "id", idTable: "t1", value: "123" }]};
        ...
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "=", table: "t1", exprs: [{ type: "id", table: "t1" }, null]});
      });

      it("allows empty lhs for prefix-type expressions", function() {
        const expr = { type: "op", op: "distance", table: "t1", exprs: []};
        return compare(this.exprCleaner.cleanExpr(expr), { type: "op", op: "distance", table: "t1", exprs: [null, null]});
      });

      it("allows null-type lhs for prefix-type expressions", function() {
        const expr = { type: "op", op: "distance", table: "t1", exprs: [{ type: "scalar", table: "t1", joins: ["1-2"], expr: null }, null] };
        return compare(this.exprCleaner.cleanExpr(expr), expr);
      });

      return it("removes invalid lhs", function() {
        const expr = { type: "op", op: "=", table: "t1", exprs: [{ type: "field", table: "t1", column: "NONSUCH" }, null]};
        return compare(this.exprCleaner.cleanExpr(expr), null);
      });
    });
    ...
    describe("score", function() {
      ...
      it("removes invalid scores keys", function() {
        const expr = { type: "score", table: "t1", input: { type: "field", table: "t1", column: "enum" }, scores: { a: 3, nonsuch: 4 } };
        return this.clean(expr, { type: "score", table: "t1", input: { type: "field", table: "t1", column: "enum" }, scores: { a: 3 } });
      });
      ...
      it("cleans score values", function() {
        ...
        return this.clean(expr, { type: "score", table: "t1", input: { type: "field", table: "t1", column: "enum" }, scores: { } });
      });
    ...
    describe("build enumset", function() {
      it("cleans values", function() {
        let expr = { type: "build enumset", table: "t1", values: { a: { type: "field", table: "t1", column: "boolean" }} };
        this.clean(expr, expr);
    ...
    return describe("scalar", function() {
      it("leaves valid one alone", function() {
        const fieldExpr = { type: "op", table: "t2", op: "sum", exprs: [{ type: "field", table: "t2", column: "number" }] };
      ...
      it("moves aggr to expr", function() {
        ...
        return compare(scalarExpr, { type: "scalar", table: "t1", joins: ['1-2'], expr: { type: "op", op: "sum", table: "t2", exprs: [fieldExpr] }});
      });
      ...
      it("defaults aggr if needed", function() {
        ...
        scalarExpr = this.exprCleaner.cleanExpr(scalarExpr);
        return compare(scalarExpr, { type: "scalar", table: "t1", joins: ['1-2'], expr: { type: "op", op: "last", table: "t2", exprs: [fieldExpr] }});
      });

      it("strips where if wrong table", function() {
        const fieldExpr = { type: "op", op: "sum", table: "t2", exprs: [{ type: "field", table: "t2", column: "number" }] };
  ...
  return describe("upgrade", function() {
    ...
    it("upgrades legacy entity join references", function() {
      ...
      const clean = (expr, expected, options) => {
