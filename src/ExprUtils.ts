...
export default class ExprUtils {
  ...
  getExprAggrStatus(expr: Expr, _depth?: number): AggrStatus | null {
    ...
    switch (expr.type) {
      ...
      case "build enumset":
        ...
        return getListAggrStatus(exprs);
      case "count": case "comparison": case "logical": // Deprecated
        return "individual";
      case "variable":
        var variable = _.findWhere(this.variables, {id: expr.variableId});
        if (!variable) {
  ...
  getAggrTypes(expr: Expr) {
    ...
    const aggrOpItems = this.findMatchingOpItems({lhsExpr: expr, aggr: true, ordered: (exprTable ? this.schema.getTable(exprTable)?.ordering != null : false)});
  ...
  static andExprs(table: string, ...exprs: Expr[]) {
    var exprsMapped = _.map(exprs, function(expr) { if ((expr?.type === "op") && (expr.op === "and")) { return expr.exprs; } else { return expr; } });
          ...
          if (opItem.prefix) {
            return (opItem.prefixLabel || opItem.name) + " " + 
              _.map(expr.exprs, (e, index) => { 
                ...
                if (index === 0) { 
                  if (e) { return this.summarizeExpr(e, locale); } else { return opItem.lhsPlaceholder || "None"; } 
                } else { 
                  if (e) { return this.summarizeExpr(e, locale); } else { return opItem.rhsPlaceholder || "None"; }
                }
  ...
  stringifyExprLiteral(expr: Expr, literal: any, locale?: string, preferEnumCodes?: boolean): string {
    return this.stringifyLiteralValue(this.getExprType(expr) as LiteralType, literal, locale, this.getExprEnumValues(expr), preferEnumCodes);
  }
  ...
  stringifyLiteralValue(type: LiteralType, value: any, locale?: string, enumValues?: EnumValue[] | null, preferEnumCodes?: boolean) {
    if ((value == null)) {
      return "None"; // TODO localize
    }
...
for (const type of (['text', 'number', 'enum', 'enumset', 'boolean', 'date', 'datetime', 'geometry', 'id'] as LiteralType[])) {
...
for (const type of ['text', 'number', 'enum', 'enumset', 'boolean', 'date', 'datetime', 'geometry', 'image', 'imagelist', 'id', 'json', 'dataurl'] as LiteralType[]) {
