...
module.exports = class ExprUtils
    ...
    return _.filter items, (opItem) =>
      ...
      if search.lhsExpr
        ...
        if lhsType
          ...
          if not opItem.exprTypes[0]? and not opItem.moreExprType?
            return false

          # If doesn't match first
          if opItem.exprTypes[0]? and opItem.exprTypes[0] != lhsType
            return false
          
          # If doesn't match more
          if not opItem.exprTypes[0]? and opItem.moreExprType? and opItem.moreExprType != lhsType
            return false
  ...
  getExprEnumValues: (expr) ->
    ...
    if expr.type == "op" and expr.op == "weekofyear"
      return [
        { id: "01", name: { en: "01" }}
        { id: "02", name: { en: "02" }}
        { id: "03", name: { en: "03" }}
...
for type in ['text', 'number', 'enum', 'enumset', 'boolean', 'date', 'datetime', 'geometry', 'image', 'imagelist', 'id', 'json', 'dataurl']
