...
# Autogenerated aliases are tricky to make as it causes SQL errors if they are not unique. The problem is compounded
# in that expressions can contain bits of other expressions compiled at an earlier time (like indicator calculations, for example
# that end up having complex jsonql expressions for fields). See https://github.com/mWater/mwater-server/issues/203 for example
# To solve this, we use globally unique aliases until a better sol'n can be found.
...
module.exports = class ExprCompiler 
  ...
  # Resets the alias number for test purposes. Ugly to have static global like this, but no other good options right now
  testResetAlias: ->
    aliasNum = 1
